...
- **Rust core** (`core/crates/lfs_core`): three Automerge document types — profile, pings, threads — behind a typed [UniFFI](https://mozilla.github.io/uniffi-rs/) surface; SQLite persistence behind a single storage adapter that tags every row with its format.
- **Shells** (`apps/ios`, `apps/android`): SwiftUI and Jetpack Compose, each opening the typed documents through the core and rendering them. Seed → kill → cold relaunch restores content from SQLite on both platforms.
- **Targets**: `aarch64-apple-ios`, `aarch64-apple-ios-sim`, `aarch64-linux-android`, `x86_64-linux-android`.
- **Dependencies of note** (see `core/crates/lfs_core/Cargo.toml`): `keyhive_core` pinned to git rev `90fe4a51`, no optional features enabled — linked and pin-verified, not yet used to issue grants; `samod` / `autosurgeon` 0.14 (Automerge 0.12); `rusqlite` 0.40 bundled; `uniffi` 0.32; `atrium-*` for atproto identity resolution; `reqwest` on `rustls`. Subduction is declared behind a feature flag and off by default.

Not yet: sync between devices, capability grants, or any identity ceremony. The next stage puts two documents under Keyhive grants; it is scoped against the current encoding, and the evidence note below records what it would cost to migrate if that encoding changes.

## Building

See [`BUILDING.md`](BUILDING.md). One `cargo make` task per platform; host tests with `cargo make test`.
...
- `docs/` also holds the load-of-effort packet and a few upstream read notes.
